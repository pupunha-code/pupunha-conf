...
import { ConferenceDay } from '@/types/event';
...
export const isDayOneSession = (date: string) => {
  try {
    return formatInTimeZone(new Date(date), VENUE_TIMEZONE, DATE_FORMAT) === DAY_ONE_DATE;
  } catch {
    return false;
  }
};
...
export const isDayTwoSession = (date: string) => {
  try {
    return formatInTimeZone(new Date(date), VENUE_TIMEZONE, DATE_FORMAT) === DAY_TWO_DATE;
  } catch {
    return false;
  }
};
...
export const getInitialDay = () => {
  const isDayTwo = formatInTimeZone(new Date(), VENUE_TIMEZONE, DATE_FORMAT) === DAY_TWO_DATE;

  return isDayTwo ? ConferenceDay.Two : ConferenceDay.One;
};

export const getCurrentConferenceDay = () => {
  const isDayOne = formatInTimeZone(new Date(), VENUE_TIMEZONE, DATE_FORMAT) === DAY_ONE_DATE;
